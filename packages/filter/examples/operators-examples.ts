...
console.log(filter(products, { price: { $gt: 100 } }).map((p) => `${p.name} ($${p.price})`));
...
console.log(filter(products, { price: { $ne: 25 } }).map((p) => `${p.name} ($${p.price})`));
...
console.log(
  filter(products, { tags: { $contains: 'computer' } }).map(
    (p) => `${p.name} (${p.tags.join(', ')})`
  )
);
